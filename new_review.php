<section style="color: #000; background-color: #fff;">
...
$query = "SELECT * FROM agenda1 ORDER BY id DESC";
...
foreach ($currentReviews as $review) {
    ...
    // Fetch the image data from the database (assuming your table has a 'profile_picture' column)
    $imageData = $review['profile_picture'];
...
$base64ImageData = base64_encode($imageData);
    ...
    echo '      <p class="mb-2">';
    echo '        <i class="fas fa-quote-left pe-2"></i>' . $review['content'] . '';
    echo '      </p>';
    echo '    </div>';
    echo '  </div>';
    echo '</div>';
